...
- [x] Component 3a: Update `.cowork/skills/references/card-templates.md` — replace alias table body with pointer to `label-aliases.json`; fix `#wp` canonical template (`そう: [value]` → `そう (looks like): [value]`)
- [x] Component 3b: Update `.cowork/skills/references/adj-forms.md` — rename `そう:` to `そう (looks like):` in all four code blocks (い-adjective, いい/よい special case, な-adjective, non-adjective); leave prose/heading references to `そう` unchanged
