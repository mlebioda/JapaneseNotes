TARGET DECK: N5-Udemy::N5-Udemy-Grammar::UN5GL5-I-will-go-to-hiroshima-next-tuesday

#w どうしのしゅるいとどうしのフォーム - types and forms of verbs  

#w 動詞（どうし） – verb
#w 種類（ しゅるい） – form, type

#w 「じしょけい」と「ますけい」をりかいする - understand the dictionary form and ます form

#w 辞書形　（じしょけい） – forma słownikowa (forma podstawowa czasownika)

#w ますけい（ます形） – forma -ます (forma uprzejma czasownika)

#w じょし「へ」「に」「を」のつかいかた - how to use particles へ, に, を


## ごい - vocabulary

#w 大阪城（おおさかじょう) - osaka castle

#w 遊園地（ゆうえんち) - amusement park

#w 出張（しゅっちょう）- a business trip K
#w 出張する（しゅっちょう）- to go on a business trip K
#w 海外に出張する（かいがい、しゅっちょう) - go to a business trip abroad
#w 京都に出張する（きょうと、しゅっちょう）- go to Kyoto on business trip

#w おみやげ - souvenir


#w ちょっと暑い（あつ）- a little hot
#w ちょっと大きい（おお）- a little big
#w ちょっと疲れました（つか）- i got little tired

#w 机の上に何がありますか（つくえ、なに）- what is on the desk?

#wc 疲れる（つか）- to get tired

#w お疲れ様でした（つか,さま）- see you tommorow, thank you for your hard work (after work, businness meeting)

#w お疲れ様です（つか、さま）- thank you for your work, w trakcie pracy przy powitaniu

#w 支社（ししゃ）- branch office
#wc 遊ぶ（あそぶ）- to play

#wc 戻る（もど）- to return, to come back, back, undo
#w 東京に戻る（とうきょう、もど）- return to tokio
#w 大阪に戻る（おおさか、もど）- return to osaka
#w 会社に戻る（かいしゃ、もど）- return to office

#w 中華料理（ちゅうかりょうり）- chinese (style) cousine

#w ご飯（はん）- meal
#w お酒（さけ）- alcohol
#w 料理（りょうり）- cooking (noun)

#wc 教える（おし）- to teach
#wc 習う（なら）- to learn

#wc 貸す（か）- to lend (to somebody)
#w いすを貸す（か) - to lend a chair

#wc 借りる（か）- to borrow (from somebody)
#w 本を借りる（か) - to borrow a book (from smb)

#wc 走る（はし）- to run
#wc 歩く（歩く）- to walk
#wc 散歩（さんぽ）- a walk/a stroll(spacer)
#wc 書く（かく）- to write

#wp だめ (な) - no good/no use/impossible
#w だめですか - is it bad?

#wp ざんねん（な）- regret/pity (żal/litość)
#w ざんねんですが - im afraid that..., unfortunately
#w ざんねんですが、今日　会議があります（かいぎ）- Unfortunately, i have a meeting today

#w レポート - report

#w 今度（こんど) - this/next time, in near future
#w 今度の料理（こんど、りょうり） - the meal this time
#w 今度の日曜日（こんど、にちようび）- next sunday
#w また、こんどね - (until) next time

#w 味噌汁（みそしる）- miso soup
#w おにぎり - rice ball
#w 天ぷら（てんぷら）-  deep fried fish and vegetables K
#w 旅行（りょこう）- trip

#wp 順調 (な）（じゅんちょう）- satisfactory/smooth (zadowalający/gładki)
#w 仕事はじゅんちょうです - my work is going well
#w 

#w また - again
#w 今日は私は学校に行きました、明日また行きます - Today i went to school, tommorow I'll go again

#w 昨日出張がありました、明日はまた出張があります（きのう、しゅっちょう) - i went on business trip yesterday, I have again businnes trip tommorow

#w 仕事を休む（やすむ) - absent from work
#w 学校を休む（学校をやすむ) - absent from school
...
#w ただいま - I'm back home (greeting used when arriving back home or someplace, like work)

#w お帰り（なさい）（かえ) - welcome home/back

#w いかがでしたか - how was it?/ Was it OK?
#w おおさかの出張いかがでしたか（しゅっちょう）- how was business trip in osaka?
#w 今日の仕事はいかがでしたか（しゅっちょう）- how was work today?

#w いいですね - Sounds good

#w ちょっと... - its a little difficult (euphemistic way of declining an invitation)

#w ざんねんですが。。。- im afraid that, im sorry but

#w 今晩、いっしょに映画館に行きますか（こんばん、えいがかん) 2. いいですね - Would you like to go out to the cinema with me tonight? 2. sounds good!

#w 今晩、お時間はありますか(こんばん) 2. ああ、今晩ちょっと。。。 - do you have time tonight? 2. aa, its a little bit difficult tonight

#w この資料はだめですか（しりょう）- Is this document no good?
#w これはだめですか - Is that bad?
## Grammar
...
#w いっしょにいきましょう - lets go together
#w 私の学校は弟の学校　と　いっしょです - my and my younger brother's school is the same (nie onaji
...
#wc 覚える（おぼ）- to remember
#wc 考える（かんがえる）- to think, zastanawiac, rozwazac, analizowac
#w よく**考（かんが）えて**ください。– Proszę dobrze się zastanowić.
#wc 思う（おもう）- to think, sądzic bardziej uczuciowo

#wc 出る（で） - to go out
#wc 着る（き） - to put on(clothes)

#wc 寝る（ねる) - to sleep
#wc 来る（くる）- to come
#w 掃除する（そうじ）- to clean
#w 電話する（でんわ）- to make a phone call
...
#w 明日だれと会うの？ - who will you meet tomorrow? (dict)
#w れんらくする - kontaktowac się, powiadomić, dać znać 
#w 連絡（れんらく）- kontakt, wiadomośc, powiadomienie

#w 部長に連絡する（ぶちょう、れんらく）-Contact the manager

#wc 出かける（で）- to go out (when u say why, ze jestes w drodze)

#w 今（いま）から**出（で）かけます**。– Wychodzę teraz (np. z domu).
    
#w 友達（ともだち）と**出（で）かけました**。– Wyszedłem z przyjacielem (gdzieś).
    
#w  母（はは）はもう**出（で）かけています**。– Mama już wyszła.
...
- Structure
	- Time + に + V
- Example
	- ９:00 **に**　会社へ行きます (くじ) - I go to the company at 9:00
	- 8月２１日**に**　大阪へ行きました（はちがつ、にじゅうににち） - I went to osaka on August 21
- Exceptions
	- 今日、明日、明後日、昨日、ー昨日
	- 今朝、今晩
	- 今
	...
	- 今年、来年、去年
	- Example
		- 明後日（あさって）学校に行きます - i will go to school the day after tomorrow
		- 来月　けっこんします - i will get married next month
- Optional
	- 昼（ひる）
	- 夜
	- 朝
	...
	- ばん
	- Examples
		- 昼（に）ゆうえんちに行きました（ひる） - i went to amusement park in the afternoon
...
#### 2. Indicates travel's starting point

- Structure
	- N＋を＋V 
- Examples
	- 会社を出る - leave the compay
	- 東京をはなれる - leave Tokio

#### 3. Indicates a transit spot and time

- Structure
	- N＋を＋V 
- Example
	- こうえんをはし走る（はし）- run in the park
...
#w 会社へ戻ります（もど）- i go back to the company
 
#w 明後日（あさって）学校に行きます - i will go to school the day after tomorrow
#w 来月　けっこんします - i will get married next month

#w 昼（に）ゆうえんちに行きました（ひる） - i went to amusement park in the afternoon
#w 夜（に）おにぎりを食べます (よる) - I will eat onigiri at night


#w 今朝みそしろ と すしを食べました - i ate miso soup and sushi this morning
#w アンドさんは　本を借ります（か）- ms Ando borrows a book

#w 会社を出る（で） - leave the compay
#w 東京をはなれる - leave Tokio

#w はなれる - leave - fizycznie/emocjonalnie opuścić, oddalić się na dlugo
#w こうえんをはし走る（はし）- run in the park

#w 会議をします(かいぎ) - to hold a meeting
#w 仕事をします - do a job
#w 来週の土曜日、何をしますか（らいしゅう、どようび）- What are you going to do next saturday?
#w 祝日、何をしました（しゅくじつ）- What did you do on your day off?
...
ない形: さんぽしない  
なかった形: さんぽしなかった  
あれる形 (passive/honorific): さんぽされる  
使役形 (make/let): さんぽさせる  
ば形 (if): さんぽすれば  
可能形 (can): さんぽできる  
おう形 (let's): さんぽしよう  
命令形 (imperative): さんぽしろ
...
ない形: かんがえない  
なかった形: かんがえなかった  
あれる形 (passive/honorific): かんがえられる  
使役形 (make/let): かんがえさせる  
ば形 (if): かんがえれば  
可能形 (can): かんがえられる  
おう形 (let's): かんがえよう  
命令形 (imperative): かんがえろ
